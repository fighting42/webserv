...
int main(int argc, char *argv[]){
    ...
    fcntl(sock_client, F_SETFL, O_NONBLOCK); // Set socket to non-blocking mode
    ...
    while(1){ // 연결 수락시 반복문
        ...
        if(read_chk == EAGAIN || read_chk == EWOULDBLOCK){
            cout << "read error" << endl;
            break;
        } else {
            r_buff[strlen(r_buff)] = '\0';
            cout << "server: " << r_buff << endl; // 버퍼 출력
        }
    }
